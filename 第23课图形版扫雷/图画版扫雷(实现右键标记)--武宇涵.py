...
class Mine:
    def __init__(self,x,y):
        ...
        self.is_mark=0#用于检测板块是否被标记,初值为0是就是未被标记
    def __str__(self):
    ...
    def mark(self):
        ...
        if self.is_fan==0 and self.is_mark==0:
            self.txt="Ⓛ"
            self.is_mark=1#用于表示该方块已经被标记
        
        elif self.is_fan==0 and self.is_mark==1:
            self.txt="■"
            self.is_mark=0#用于表示该方块已经未被标记
...
while sta==1:
    ...
    t.onscreenclick(click_right_fan,btn=3)
    if u==1:
        #game_again.clear()
        sta=0
        t2.clear()
