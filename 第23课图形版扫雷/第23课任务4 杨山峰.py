...
class MineGame:
    ...
    def on_right_click(self, x, y):
        ...
        if 0 <= row < BOARD_SIZE and 0 <= line < BOARD_SIZE:
            block = self.board[row][line]
            if not block.opened:  # 只有未被点击的方块可以标记
                block.flagged = not block.flagged  # 切换标记状态
                self.draw_board()
    ...
    def game_start(self):
        self.place_mines()
        self.draw_board()
        turtle.onscreenclick(self.on_click, 1)  # 左键点击
        turtle.onscreenclick(self.on_right_click, 3)  # 右键点击
